...
import EnhancedDataVisualizationPanel from './EnhancedDataVisualization';
import EnhancedDailyReportsPanel from './EnhancedDailyReportsPanel';
import AIReportAssistant from './AIReportAssistant';
import MediaManagementSystem from './MediaManagementSystem';
import DualLogoHeaderCustomizer from './DualLogoHeaderCustomizer';
import QuickIncidentReport from './QuickIncidentReport';
import AdvancedReportDelivery from './AdvancedReportDelivery';
...
const ReportsIntegration: React.FC<ReportsIntegrationProps> = ({
  ...
  useEffect(() => {
    if (client?.contactEmail) {
      ...
      setDeliveryOptions(prev => ({
        ...prev,
        emailRecipients: [client.contactEmail]
      }));
    }
  }, [client]);
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      if (headerImage) {
        ...
        pdf.setGlobalAlpha(themeSettings.backgroundOpacity || 0.5);
        pdf.rect(0, 0, 210, 40, 'F');
        pdf.setGlobalAlpha(1);
      } else {
        // Create a colored header background if no image
        pdf.setFillColor(themeSettings.primaryColor ? hexToRgb(themeSettings.primaryColor).r : 0, 
                         themeSettings.primaryColor ? hexToRgb(themeSettings.primaryColor).g : 112,
                         themeSettings.primaryColor ? hexToRgb(themeSettings.primaryColor).b : 243);
        pdf.rect(0, 0, 210, 40, 'F');
      }
  ...
  const handleIncidentReport = async (incidentData: any) => {
    if (onIncidentReport) {
      try {
        await onIncidentReport(incidentData);
    ...
    <>
      ...
      {showIncidentReport && (
        <div style={{ 
          ...
          <QuickIncidentReport 
            clientId={client?.id}
            onReportSent={handleIncidentReport}
