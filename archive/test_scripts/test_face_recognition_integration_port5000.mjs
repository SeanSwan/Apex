/**
 * FACE RECOGNITION INTEGRATION TEST SUITE - PORT 5000
 * ===================================================
 * Updated for backend running on port 5000
 */
...
// API Endpoint Tests - Updated for Port 5000
async function testApiEndpoints() {
  log('Testing Face Recognition API endpoints on port 5000...', 'API');
...
// Frontend Tests - Updated for both port 3000 and 5000
async function testFrontendIntegration() {
  ...
  try {
    ...
    // Test 6: Frontend Accessibility (check both ports)
    try {
      let frontendPort = null;
      
      // Try port 3000 first
      try {
        const response3000 = await fetch('http://localhost:3000');
        if (response3000.ok) {
          frontendPort = 3000;
        }
      } catch (e) {}
      
      // Try port 5000 if 3000 failed
      if (!frontendPort) {
        try {
          const response5000 = await fetch('http://localhost:5000');
          if (response5000.ok) {
            frontendPort = 5000;
          }
        } catch (e) {}
      ...
      tests.push({
        ...
        success: !!frontendPort,
        details: frontendPort ? `Frontend accessible on port ${frontendPort}` : 'Frontend not accessible'
      });
    } catch (error) {
      tests.push({
        test: 'Frontend Accessibility',
        success: false,
        details: 'Could not test frontend accessibility'
      });
    }
...
// Database Integration Tests (unchanged)
...
// AI Engine Integration Tests (unchanged)
...
function generateIntegrationReport() {
  const report = {
    ...
    accessUrls: {
      frontend: 'http://localhost:5000',  // Updated for port 5000
      faceManagement: 'http://localhost:5000/face-management',
      apiBase: 'http://localhost:5000/api',
      faceApi: 'http://localhost:5000/api/faces'
    }
  };
...
function displayIntegrationResults(report) {
  console.log('\n' + '='.repeat(100));
  console.log('🧪 APEX AI FACE RECOGNITION SYSTEM - INTEGRATION TEST RESULTS (Port 5000)');
  ...
  console.log('\n🌐 SYSTEM ACCESS URLS');
  console.log('-'.repeat(60));
  console.log(`🎯 Face Management: ${report.accessUrls.faceManagement}`);
  console.log(`🏠 Frontend: ${report.accessUrls.frontend}`);
  console.log(`📡 API Base: ${report.accessUrls.apiBase}`);
  console.log(`🧠 Face API: ${report.accessUrls.faceApi}`);
  ...
  switch (report.deploymentStatus) {
    case 'PRODUCTION_READY':
      ...
      console.log(`🎯 Access Face Management at: ${report.accessUrls.faceManagement}`);
      break;
    case 'MOSTLY_READY':
...
async function runIntegrationTests() {
  console.log('🧪 APEX AI FACE RECOGNITION - INTEGRATION TEST SUITE (Port 5000)');
  console.log('='.repeat(100));
  ...
  try {
    ...
    const reportPath = path.join(__dirname, `face_recognition_integration_test_port5000_${Date.now()}.json`);
